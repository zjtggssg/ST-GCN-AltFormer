...
from model.ST_TR.spatial_transformer import spatial_attention
...
'''在空间变压器情况下实现扩充图空间卷积的单元
In_channels:卷积单元中输入的通道数(第一个是3，因为它对应坐标的数量)
Out_channels:输出的通道数(最后一个是256)
incidence:整个骨架的关联矩阵
incidence是一个PxVxV矩阵，其中p是特定分区策略的分区数
'''
