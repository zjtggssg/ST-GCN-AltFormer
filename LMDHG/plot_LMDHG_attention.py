...
def init_model_st():
    ...
    model_path = '/home/zjt/Desktop/ST-GCN-AltFormer/LMDHG/weight/ST_TS/st.pth'
...
def plot_cam_pic_mutil(normalized_matrix,style,pd = None,emsemble = None):
    ...
    plt.ylabel("joints")
    plt.yticks(np.arange(0, normalized_matrix.shape[1], 1))
    plt.savefig('/data/zjt/LMDHG/gesture/{}/{}_{}_{}.png'.format(emsemble,style,selected_index,pd))
...
if __name__ == '__main__':
    ...
    if npy == 1:
        if pd == 'st_ts':
            matrix = np.load(
                '/data/zjt/HandGestureDataset_SHREC2017/gesture/mutil/{}/{}_{}_{}.npy'.format(pd, style, selected_index,
                                                                                              pd))
        ...
        if pd == 'st':
            ...
            np.save(
                '/data/zjt/LMDHG/gesture/{}/{}_{}_{}.npy'.format(pd, style, selected_index, pd),
                normalized_matrix)
            # plot_cam_pic_mutil(normalized_matrix, style, pd =  pd,emsemble = emsemble)
            plot_cam_pic(normalized_matrix, style, pd)