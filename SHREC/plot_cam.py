...
def init_data_loader(data_cfg):
    ...
    all_data = [sample for sample in all_data if sample["label"] == 4]
...
def plot_cam_pic(normalized_matrix,style,pd = None):
    ...
    plt.title("Tap_{}".format(selected_index))

    plt.xlabel("frames")

    plt.ylabel("joints")
    plt.savefig('/data/zjt/HandGestureDataset_SHREC2017/gesture/ST-TS/{}_{}_{}.png'.format(style,selected_index,pd))
...
if __name__ == '__main__':
    ...
    # total = sum(l)
    #
    # normalized = [x / total for x in l]

    normalized = np.array(l).reshape(zeros_start, 22)
    np.save('/data/zjt/HandGestureDataset_SHREC2017/gesture/ST-TS/{}_{}_st-ts.npy'.format(style,selected_index),normalized)

    normalized_st = np.array(l_st).reshape(zeros_start, 22)
    np.save('/data/zjt/HandGestureDataset_SHREC2017/gesture/ST-TS/{}_{}_st.npy'.format(style,selected_index), normalized_st)

    normalized_ts = np.array(l_ts).reshape(zeros_start, 22)
    np.save('/data/zjt/HandGestureDataset_SHREC2017/gesture/ST-TS/{}_{}_ts.npy'.format(style,selected_index), normalized_ts)

    normalized_matrix = normalize(normalized)
    normalized_matrix_st = normalize(normalized_st)
    normalized_matrix_ts = normalize(normalized_ts)

    plot_cam_pic(normalized_matrix,style,pd = 'st_ts')
    plot_cam_pic(normalized_matrix_st,style,pd = 'st')
    plot_cam_pic(normalized_matrix_ts,style,pd = 'ts')
