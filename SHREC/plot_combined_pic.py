...
def init_data_loader(data_cfg):
    ...
    all_data = [sample for sample in all_data if sample["label"] == 9]
...
if __name__ == '__main__':
    ...
    if switch == 1:
        # 获取数据和标签
        model = init_model_st(args.data_cfg)
        ...
        np.save('/data/zjt/HandGestureDataset_SHREC2017/gesture/ST-TS/{}_{}_{}.npy'.format(style,selected_index,pd),normalized_matrix)
        plot_cam_pic(normalized_matrix, style, pd)


    if switch == 2:
        # 获取数据和标签
        model = init_model_ts(args.data_cfg)
        sample_batched = test_loader.dataset[selected_index]  # 通过索引获取数据集中的样本
        ...
        normalized_matrix = normalize(normalized)
        np.save('/data/zjt/HandGestureDataset_SHREC2017/gesture/ST-TS/{}_{}_{}.npy'.format(style, selected_index, pd),
                normalized_matrix)
        plot_cam_pic(normalized_matrix, style, pd)

    if switch == 3:
        st = np.load('/data/zjt/HandGestureDataset_SHREC2017/gesture/ST-TS/{}_{}_{}.npy'.format(style, selected_index, 'st'))
        ts = np.load(
            '/data/zjt/HandGestureDataset_SHREC2017/gesture/ST-TS/{}_{}_{}.npy'.format(style, selected_index, 'ts'))

        combined = st + ts

        combined_matrix = normalize(combined)

        plot_cam_pic(combined_matrix, style, pd = 'st_ts')
