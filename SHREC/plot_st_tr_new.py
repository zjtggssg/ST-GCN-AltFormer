...
def plot_cam_pic(normalized_matrix,style,pd = None):
    ...
    plt.savefig('/data/zjt/HandGestureDataset_SHREC2017/gesture/{}/{}_{}_{}_new.png'.format(pd,style,selected_index,pd))
        ...
        if pd == 'str':
            ...
            ttr_matrix = np.load(
                '/data/zjt/HandGestureDataset_SHREC2017/gesture/{}/{}_{}_{}_new.npy'.format(pd2, style, selected_index,
                                                                                        pd2))
            str_matrix = np.abs(str_matrix)
            ttr_matrix = np.abs(ttr_matrix)
            st_tt_matrix =  0.5 *str_matrix +  0.5 * ttr_matrix
            # normalized_matrix = normalize(st_tt_matrix)
            np.save(
                '/data/zjt/HandGestureDataset_SHREC2017/gesture/{}/{}_{}_{}_new.npy'.format(pd, style, selected_index, pd),
                st_tt_matrix)
            plot_cam_pic(st_tt_matrix, style, pd)