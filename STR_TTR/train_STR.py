...
def init_model(data_cfg):
    ...
    graph_args = dict(l)
    address = 'graph.SHRE'
    model = ST_GCN_STR(channel=3, num_class=class_num, window_size=300, num_point=22, attention=True,
                      only_attention=True,
...
if __name__ == "__main__":
    ...
    model_fold = "/data/zjt/handgesture/STR/SHREC_dp-{}_lr-{}_dc-{}/".format(args.dp_rate,
                                                     args.learning_rate,
                                                      args.data_cfg)
    try:
        os.mkdir(model_fold)
